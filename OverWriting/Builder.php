...
class Builder extends BuilderQuery {
    ...
    protected function runPaginationCountQuery( $columns = ['*'] ) : array {
        ...
        $query = $this -> cloneWithout( [ 'orders' , 'limit' , 'offset' ] ) -> cloneWithoutBindings( [ 'order' ] );

        // We don't need simple columns, only specials
        // like subselects which is why we're using
        // havings after all.
        foreach ( $query -> columns ?? [ ] as $key => $value ) if ( is_string( $value ) ) unset( $query -> columns[ $key ] ) ;
        ...
        // checking for it.
        if ( ! $this -> groups ) $countQuery -> setAggregate( 'count' , $this -> withoutSelectAliases( $columns ) );
